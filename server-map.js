...
app.get('/api/active-users', async (req, res) => {
	try {
		...
		const speedCalculationWindow = 120 // 1 минута в секундах
			...
			.aggregate([
				...
				},
				{
					$group: {
						...
						lastLocation: {
							$last: { latitude: '$latitude', longitude: '$longitude' },
						},
		...
		activeUsers.forEach(user => {
			const recentCoordinates = user.coordinates
				.filter(
					coord => coord.timestamp >= currentTimestamp - speedCalculationWindow
				)
			...
			user.averageSpeed =
				totalTime > 0 ? totalDistance / 1000 / (totalTime / 3600) : 0 // км/ч
		...
		activeUsers.forEach(user => {
			if (user.avatarUrl) {
				user.avatarUrl = `${apiBaseUrl}/secure-avatar/${encryptUrl(
					user.avatarUrl
				)}`
			}
		})
...
app.post('/api/charging-stations/:id/like/:userId', async (req, res) => {
	try {
		...
		const dislikedBy = Array.isArray(station.dislikedBy)
			? station.dislikedBy
			: []
...
app.get('/api/monthly-heatmap/:year/:month', async (req, res) => {
	try {
		...
		if (!result) {
			return res
				.status(404)
				.json({ error: 'Heatmap data not found for the specified month' })
		}
...
const calculateStats = async (userId, startTimestamp, endTimestamp) => {
	...
	let dailyStats = Array(7)
		.fill()
		.map(() => ({ distance: 0, time: 0 }))
...
const getTopLongestSessions = async (period, limit) => {
	...
	const result = await collection
		.aggregate([
			...
			},
			{
				$addFields: {
					sessionDuration: {
						$divide: [{ $subtract: ['$lastTimestamp', '$firstTimestamp'] }, 60],
					}, // in minutes
				},
			},
			{
	...
	const topSessions = result.map(session => {
		...
		for (let i = 1; i < coordinates.length; i++) {
			...
			const timeDiff = curr.timestamp - prev.timestamp
			if (timeDiff < 3600) {
				// If the time difference is less than 1 hour, it's considered part of the session
				const distance = haversine(
					{ lat: prev.latitude, lon: prev.longitude },
					{ lat: curr.latitude, lon: curr.longitude }
				)
...
const getTopDailyDistances = async (period, limit) => {
	...
	dailyDistances.forEach(entry => {
		if (
			!uniqueUsers[entry.userId] ||
			uniqueUsers[entry.userId].distance < entry.distance
		) {
		...
		case 'last_week':
			{
				...
				lastMonday.setDate(
					now.getDate() - (dayOfWeek === 0 ? 13 : dayOfWeek + 6)
				)
				lastMonday.setHours(0, 0, 0, 0)
				const lastSunday = new Date(lastMonday)
				lastSunday.setDate(lastMonday.getDate() + 6)
...
app.get('/api/user-category-by-distance/:userId', async (req, res) => {
	try {
		...
		const userCoordinates = await collection
			.find({ userId: userId })
			.sort({ sessionId: 1, timestamp: 1 })
			.toArray()
...
app.get('/api/workshops', async (req, res) => {
	try {
		...
		console.error('Ошибка при получении списка мастерских:', error)
		res.status(500).json({ error: 'Внутренняя ошибка сервера' })
	}
})
